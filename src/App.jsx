  ...
  return (
    <div className="wm">
      ...
      <footer className="ft">
        ...
        <p className="tiny ft-disclaimer">המודעות באתר — תוכנן, המוצרים או השירותים המפורסמים בהן והקישורים המצורפים אליהן — באחריות המפרסמים בלבד. "מי ומה" הוא פלטפורמה להצגת מודעות, אינו צד לעסקה בין מפרסם לגולש ואינו אחראי לתוכן המודעות. נתקלתם במודעה פוגענית? <a href={`https://wa.me/${waNumber(CONTACT.whatsapp)}?text=${encodeURIComponent("היי, אני רוצה לדווח על מודעה באתר מי ומה:")}`} target="_blank" rel="noopener noreferrer">דווחו לנו</a>.</p>
      </footer>
      <AccessibilityMenu />
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {sent ? (
          ...
          <div className="row2">
            ...
            <label className="agree tiny"><input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} /> אני מאשר/ת שהתוכן, הקישור והמוצר/השירות המפורסם באחריותי המלאה, ושקראתי את <a href="/terms" target="_blank" rel="noopener noreferrer">תנאי השימוש</a>.</label>
            <button className="cta go" disabled={busy || !session || !file || !linkCheck?.ok || !agree} onClick={submit}>
...
function Terms() {
  return (
    <main className="doc">
      ...
      <h3>3. אחריות המפרסם ושיפוי</h3>
      <p>3.1 המפרסם אחראי באופן בלעדי ומלא לכל תוכן המודעה — לרבות התמונה, הטקסט, הקישור המצורף לה, האתר או העמוד שאליו מפנה הקישור, וכל תוכן, מוצר או שירות המוצג, משווק או מופץ באמצעותם (להלן: "תוכן המפרסם").</p>
      <p>3.2 המפרסם מצהיר ומתחייב כי תוכן המפרסם חוקי, אמין, אינו מטעה, ואינו פוגע או מפר זכות כלשהי של צד שלישי — לרבות זכויות יוצרים, סימני מסחר, סודות מסחריים, הזכות לפרטיות ולשם טוב — וכי בידיו כל ההיתרים, הרישיונות וההסכמות הנדרשים לפי כל דין לפרסום תוכן המפרסם ולהפעלת האתר שאליו מפנה הקישור.</p>
      <p>3.3 מפעילת האתר אינה בודקת, מאמתת או מפקחת באופן שוטף על תוכן המפרסם או על האתר שאליו מפנה הקישור, לרבות שינויים שנעשו בו לאחר מועד האישור. אישור מודעה לפרסום אינו מהווה אימות, המלצה, ערובה או הבעת עמדה מכל סוג באשר לתוכן המפרסם, למוצר או לשירות המוצע בו.</p>
      <p>3.4 המפרסם ישפה ויפצה את מפעילת האתר, מיד עם דרישתה הראשונה, בגין כל נזק, הפסד, תשלום, קנס או הוצאה (לרבות שכר טרחת עורך דין והוצאות משפט) שייגרמו לה כתוצאה מתביעה, דרישה, טענה או חקירה של צד שלישי כלשהו (לרבות רשות מוסמכת) בקשר עם תוכן המפרסם, המוצר או השירות המפורסם, או האתר שאליו מפנה הקישור. מפעילת האתר תודיע למפרסם על תביעה כאמור, והמפרסם רשאי להשתתף בהגנה מפניה בתיאום עמה.</p>
      ...
      <h3>5. שימוש לרעה באתר או בגולשיו</h3>
      <p>5.1 חל איסור מוחלט על שימוש באתר, במודעה או בקישור המצורף לה באופן העלול לפגוע באתר, בפעילותו התקינה, במפעילת האתר או בגולשי האתר — לרבות (ולא רק): ניסיונות פישינג או התחזות; הפצת נוזקות, וירוסים או קוד זדוני; הונאה, גניבת זהות או קידום תרמיות; איסוף או קצירת מידע ופרטים אישיים של גולשים ללא הסכמתם; הטרדה, איום או כל פעולה הפוגעת בפרטיות, בשם הטוב או בביטחון גולשי האתר.</p>
      <p>5.2 בכל מקרה של הפרה, או חשד סביר להפרה, של סעיף זה — רשאית מפעילת האתר <b>להסיר את המודעה לאלתר וללא התראה מוקדמת</b>, לחסום את גישת המפרסם לאתר, ולנקוט בכל פעולה הנדרשת למניעת המשך הפגיעה.</p>
      <p>5.3 מפעילת האתר רשאית להעביר את פרטי המפרסם ואת פרטי ההפרה לרשויות אכיפת החוק ולצדדים שנפגעו, ולנקוט בכל הליך משפטי, אזרחי או פלילי, נגד המפרסם.</p>
      <p>5.4 <b>מפרסם שהפר את הוראות סעיף זה לא יהיה זכאי להחזר כספי כלשהו</b>, מלא או חלקי, בגין המודעה שהוסרה — יהא מועד ההסרה אשר יהא, וזאת על אף כל הוראה אחרת בתקנון זה בדבר ביטול והחזרים.</p>
      <p>5.5 המפרסם יישא בכל עלות, הוצאה או נזק שייגרמו למפעילת האתר כתוצאה מההפרה, לרבות עלויות טיפול, הסרה וייצוג משפטי, בנוסף לכל סעד אחר על פי דין.</p>
      <p>5.6 נתקלתם במודעה או בקישור שפוגעים בכם? ניתן לדווח למפעילת האתר בטלפון <span dir="ltr">{CONTACT.phone}</span> או בדוא"ל <a href={`mailto:${CONTACT.email}`} dir="ltr">{CONTACT.email}</a>. דיווחים נבדקים בהקדם, ומודעה שנמצאה מפרה מוסרת.</p>

      <h3>6. אישור, תשלום ועדכונים</h3>
      ...
      <h3>7. תוקף המודעה וויתור על מקום</h3>
      ...
      <h3>9. אין התחייבות לחשיפה או לתוצאות</h3>
